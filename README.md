...
| Idle | Syncing | Failed |
| --- | --- | --- |
| ![idle](docs/images/toolbar-idle.png) | ![syncing](docs/images/toolbar-syncing.png) | ![error](docs/images/toolbar-error.png) |

## What it is not

It is not a replacement for Zotero's own data sync, and not a two-way sync engine. The
authoritative copy lives in Zotero; the repository is a versioned, readable mirror. Edits
made on GitHub are overwritten by the next sync, and *Import from GitHub* only ever adds or
refreshes — it never deletes anything locally.
