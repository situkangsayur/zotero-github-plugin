...
## 7. Syncing from more than one computer

The plugin remembers what each computer last synced, so it can tell your changes from
changes made on GitHub or by another computer. When a sync finds something that needs a
decision, it opens a review inside the Zotero window:

![Review panel](images/review-panel.png)

| Section | What it means | Your choice |
| --- | --- | --- |
| **Changes on GitHub** | Edited or added elsewhere, unchanged here | Tick to import into Zotero; unticked items are asked about again next time |
| **Changed in both places** | Edited here *and* elsewhere | Keep Zotero's version, take GitHub's, or — for files — keep both |
| **Deleted on GitHub, still in Zotero** | Another computer deleted it | Tick to upload it again. To accept the deletion, delete the item in Zotero |
| **Edited on GitHub, generated by the plugin** | Someone edited a Markdown note or index on GitHub | These are regenerated from your library; untick to leave them alone this time |
| **Files this sync replaces or deletes on GitHub** | Your own changes | Untick a file to exclude it from this sync |

Click **Sync** to apply your choices, or **Cancel** (or press Escape) to change nothing.

Background syncs never wait for you. They upload what is safe and leave everything else as
it is; the button shows an orange dot:

![Toolbar button with changes waiting for review](images/toolbar-attention.png)

Click it, or use **Tools → GitHub Sync → Review Changes and Sync…**, to review. That menu item
also shows the review when nothing needs a decision, so you can see — and exclude — exactly
which files a sync is about to replace or delete.

## 8. Restore on another computer
...
- **Zotero stays the source of truth.** Changes made on GitHub reach your library only when
  you accept them in the review, and nothing is ever deleted from your library by a sync.
